...
def main():
    ...
    print(f'Searching {args} for {pattern}')
    excludes = set()
    for path in args:
    ...
    print('Writing to \'generated_excludes.txt\'')
...
def dig_path(cwd, excludes, pattern):
    ...
    try:
        to_be_excluded = [ Path(cwd.root, cwd) for i in cwd.iterdir() if re.match(pattern, i.name) != None ]
